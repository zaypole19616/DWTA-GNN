...
class ExternalNodeTextField(Field): ###
...
class ACE2005Dataset(Corpus):
    ...
    def parse_example(self, path, fields): ## 一个文档是一个example
        examples = []
        with open(path, "r", encoding="utf-8") as f: # path: training.jl,
            for line in f: # 每一个line为一篇文档（list），list里的每一个dict为一句sentence
                line = line.strip()
                if len(line) == 0:
                    continue
                jl = json.loads(line, encoding="utf-8") # jl: 由json load转化好的一个list
                for js in jl: # js: 每一句Sentence，为一个dict
                    # print(js)
    ...
    def parse_sentence(self, js, fields):
        ...
        EXTERNALNODES = fields["external-nodes"]
        EXTERNALNODES_TEXT = fields["external-nodes-text"]
        ALLNODES = fields["all-nodes"]
        ALLNODES_TEXT = fields["all-nodes-text"]
        SENTENCES_MAP = fields["sentences-map"]
        ...
        setattr(ex, EXTERNALNODES[0], EXTERNALNODES[1].preprocess(sentence.externalNodeList))
        setattr(ex, EXTERNALNODES_TEXT[0], EXTERNALNODES_TEXT[1].preprocess(sentence.externalNodeList))
        setattr(ex, ALLNODES[0], ALLNODES[1].preprocess(sentence.allNodeList))
        setattr(ex, ALLNODES_TEXT[0], ALLNODES_TEXT[1].preprocess(sentence.allNodeList))
        setattr(ex, SENTENCES_MAP[0], SENTENCES_MAP[1].preprocess(sentence.sentenceMap))
