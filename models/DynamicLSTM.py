...
class DynamicLSTM(nn.Module):
    ...
    def forward(self, x, x_len, only_use_last_hidden_state=False):
        ...
        ### 用gpu来处理：
        x_sort_idx = torch.argsort(-x_len)  # Returns the indices that would sort an array.
        x_unsort_idx = torch.cuda.LongTensor(torch.argsort(x_sort_idx)).to(self.device)
        x_len = x_len[x_sort_idx]
        x = x[torch.cuda.LongTensor(x_sort_idx).to(self.device)]
        # 2. pack
