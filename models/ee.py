...
class EDModel(Model):
    def __init__(self, hyps, device=torch.device("cpu"), embeddingMatrix=None):
        ...
        for i in range(hyps["gcn_layers"]):
            gcn = GraphConvolution(in_features=hyps["ex_node_dim"],
                                   out_features=hyps["ex_node_dim"],
        ...
        if hyps["use_highway"]:
            ...
            for i in range(hyps["gcn_layers"]):
                hw = HighWay(size=hyps["ex_node_dim"], dropout_ratio=hyps["gcn_dp"])
                self.hws.append(hw)
    ...
    def get_sentence_positional_feature(self, BATCH_SIZE, SEQ_LEN):
        positions = [[abs(j) for j in range(-i, SEQ_LEN - i)] for i in range(SEQ_LEN)]
    ...
    def forward(self, word_sequence, x_len, pos_tagging_sequence, entity_type_sequence, adj, batch_golden_entities,
        ...
        :param label_i2s:
        :param ex_nodes: FloatTensor, padded external node indices
        :return:
        ...
        x_emb = torch.cat([word_emb, pos_emb, entity_label_emb], 2)
        ...
        for i in range(SEQ_LEN):
            x, _ = self.bilstm(torch.cat([x_emb, self.psembeddings(positional_sequences[i].to(self.device))], 2), x_len)

            batch_sent_nodes = []
            for b in range(BATCH_SIZE):
                sen_x_len = x_len[b].tolist()
                ex_nodes = all_nodes[b]

                if sen_x_len == SEQ_LEN:
                    sen_word = x[b]
                    sen_nodes = torch.cat((sen_word, ex_nodes), dim=0)
                    batch_sent_nodes.append(sen_nodes)
                else:
                    sen_word = x[b][:sen_x_len]
                    sen_word_pad = x[b][sen_x_len:]
                    sen_nodes = torch.cat((sen_word, ex_nodes, sen_word_pad), dim=0)
                    batch_sent_nodes.append(sen_nodes)

            batch_sent_nodes = torch.stack(batch_sent_nodes, dim=0)

            for l in range(self.hyperparams["gcn_layers"]):
                if self.hyperparams["use_highway"]:
                    batch_sent_nodes = self.gcns[l](batch_sent_nodes, adj) + self.hws[l](batch_sent_nodes)
                else:
                    batch_sent_nodes = self.gcns[l](batch_sent_nodes, adj)

            x = batch_sent_nodes[:, :SEQ_LEN, :]

            xx.append(self.sa(x, mask))

        xx = torch.stack(xx, dim=1)

        logits = self.ol(xx)

        ae_hidden = []
        ae_logits_key = []
        trigger_outputs = torch.max(logits, 2)[1].view(logits.size()[:2])

        for i in range(BATCH_SIZE):
            predicted_event_triggers = EDTester.merge_segments([label_i2s[x] for x in trigger_outputs[i][:x_len[i]].tolist()])
            ...
            for j in range(len(golden_entities)):
                ...
                try:
                    golden_entity_tensors[golden_entities[j]] = xx[i, e_st:e_ed, ].mean(dim=0)
                except:
                    print(xx.size())
                    print(e_st, e_ed)
            ...
            for st in predicted_event_triggers:
                ...
                for j in range(len(golden_entities)):
                    ...
                    ae_hidden.append(torch.cat([event_tensor, entity_tensor]))
                    ae_logits_key.append((i, st, ed, trigger_type_str, e_st, e_ed, e_type_str)) # [(0, 0, 1, "MOVEMENT||TRANSPORT", 39, 40, 'PER'), ...]
