...
from enet.corpus.Data import ACE2005Dataset, MultiTokenField, SparseField, EventField, EntityField, ExternalNodeTextField, AllNodeTextField, SentenceMapField
...
class EERunner(object):
    ...
    def run(self):
        ...
        print("batch size: ", self.a.batch)
        print("learning rate: ", self.a.lr)
        ...
        ExternalNodesField = Field(lower=True, include_lengths=True, batch_first=True)
        ExternalNodesTextField = ExternalNodeTextField(lower=False, batch_first=True, include_lengths=True, use_vocab=False)
        AllNodesField = Field(lower=True, include_lengths=True, batch_first=True)
        AllNodesTextField = AllNodeTextField(lower=False, batch_first=True, include_lengths=True, use_vocab=False)
        SentencesMapField = SentenceMapField(lower=False, batch_first=True, include_lengths=True, use_vocab=False)
        ...
        train_set = ACE2005Dataset(path=self.a.train,
                                   fields={"words": ("WORDS", WordsField),
                                           ...
                                           "external-nodes": ("EXTERNALNODES", ExternalNodesField),
                                           "external-nodes-text": ("EXTERNALNODES_TEXT", ExternalNodesTextField),
                                           "all-nodes": ("ALLNODES", AllNodesField),
                                           "all-nodes-text": ("ALLNODES_TEXT", AllNodesTextField),
                                           "sentences-map": ("SENTENCES_MAP", SentencesMapField)
                                           },
                                   keep_events=1)
        ...
        ExternalNodesField.build_vocab(train_set.EXTERNALNODES, dev_set.EXTERNALNODES)
        AllNodesField.build_vocab(train_set.ALLNODES, dev_set.ALLNODES)
        ...
        self.a.event_arg_weight = torch.ones([len(EventsField.vocab.itos)]) * 5
        self.a.event_arg_weight[consts.ROLE_O_LABEL] = 1.0
        ...
        if "ex_node_dim" not in self.a.hps:
            self.a.hps["ex_node_dim"] = 440

        print("[runner.py] oc: ", self.a.hps["oc"])
        print("[runner.py] LabelField.vocab.itos: ", LabelField.vocab.itos) #['O', 'B-CONFLICT||ATTACK', 'B-PERSONNEL||ELECT', 'B-PERSONNEL||END|POSITION']
        print("[runner.py] ae_oc: ", self.a.hps["ae_oc"])
        print("[runner.py] EventsField.vocab.itos: ", EventsField.vocab.itos)

        tester = self.get_tester(LabelField.vocab.itos) # tesing.py
        ...
        self.a.ex_node_i2s = ExternalNodesField.vocab.itos
        self.a.all_node_i2s = AllNodesField.vocab.itos
