...
def train(model, train_set, dev_set, test_set, optimizer_constructor, epochs, tester, parser, other_testsets):
    train_iter = BucketIterator(train_set, batch_size=parser.batch, train=False, shuffle=True, device='cpu',
                                  sort_key=lambda x: len(x.POSTAGS))
    dev_iter = BucketIterator(dev_set, batch_size=parser.batch, train=False, shuffle=True, device='cpu',
                              sort_key=lambda x: len(x.POSTAGS))
    test_iter = BucketIterator(test_set, batch_size=parser.batch, train=False, shuffle=True, device='cpu',
                               sort_key=lambda x: len(x.POSTAGS))
    ...
    # loading transE
    print("Loading external nodes embedding...\n")
    transe_vec_dir = os.path.dirname(os.path.realpath(__file__)).replace("/enet", "/embedding/TransE/")
    transE_json_str = open(transe_vec_dir + "node2vec.json", 'r', encoding='utf-8').read()
    node2vec = json.loads(transE_json_str)

    for i in range(epochs):
        # Training Phrase
        print("\nEpoch", i + 1)
        training_loss, training_ed_p, training_ed_r, training_ed_f1, training_ae_p, training_ae_r, training_ae_f1 = run_over_data(
                                                                     ...
                                                                     all_node_i2s=parser.all_node_i2s,
                                                                     ex_node_i2s=parser.ex_node_i2s,
                                                                     ex_node2vec=node2vec,
                                                                     weight=parser.label_weight,
                                                                     arg_weight=parser.event_arg_weight,
                                                                     save_output=os.path.join(parser.out, "training_epoch_%d.txt" % (i + 1))
                                                                     )
        ...
        with torch.no_grad():
            # Validation Phrase
            dev_loss, dev_ed_p, dev_ed_r, dev_ed_f1, dev_ae_p, dev_ae_r, dev_ae_f1 = run_over_data(data_iter=dev_iter,
                                                        optimizer=optimizer,
                                                        model=model,
                                                        need_backward=False,
                                                        ...
                                                        weight=parser.label_weight,
                                                        arg_weight=parser.event_arg_weight,
                                                        save_output=os.path.join(parser.out, "dev_epoch_%d.txt" % (i + 1)))
            ...
            # Testing Phrase
            test_loss, test_ed_p, test_ed_r, test_ed_f1, test_ae_p, test_ae_r, test_ae_f1 = run_over_data(data_iter=test_iter,
                                                            optimizer=optimizer,
                                                            model=model,
                                                            need_backward=False,
                                                            ...
                                                            weight=parser.label_weight,
                                                            arg_weight=parser.event_arg_weight,
                                                            save_output=os.path.join(parser.out, "test_epoch_%d.txt" % (i + 1)))
